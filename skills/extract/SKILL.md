...
> **全流程**：提取（extract）→ 验证（verify）→ 参数扫描（param_scan）→ 整理进库（organizer）→ 沉淀。
> 脚本在 `scripts/`，引擎 MinerU 外部引用（`MINERU_BIN` 指向 mineru.exe）。
> **脚本管机械，agent 管理解**——据图补全、参数提炼、规范整理必须 LLM 亲自做，不许脚本代劳。
...
| 3 参数扫描 | 扫出参数映射表候选 + 图-only 表 | 脚本 | `python scripts/param_scan.py <产物目录>` |
| 4 整理 | 写笔记 + 据图补全 + 保留公式 + 参数映射表/流程引导 | 代理 | `dispatch agents/extract-organizer.md` |
| 5 沉淀 | 建索引 + git 提交 | 脚本/代理 | 按知识库规范 |
...
1. 脚本是机械骨架，理解（据图补全、参数提炼、规范整理）必须 LLM/代理亲自做。
...
4. **引擎外部引用不粘代码**：MinerU 是黑盒 subprocess 调（源码验证：backend↔model 互相耦合，无法拆出单独复用）。
5. **完整利用产物**：content_list.json（bbox/text_level/page_idx）是结构化层，别只用 markdown。
